...
- **[mcpaudit](https://github.com/AndrewXuTurtle/mcpaudit)** 🟢 — Config-side auditor for MCP servers already installed across Claude Desktop, Claude Code, Cursor, Windsurf and VS Code. Weights **provenance** alongside content: homoglyph publisher scopes, packages the registry replaced with a security placeholder, install hooks, and OSV/GitHub advisories matched against the *resolved* version rather than the package name. Zero runtime dependencies (hand-written tar reader); nothing is installed or executed. Also runs as an MCP server (`--mcp`) so an agent can audit its own configuration, and publishes a daily-rebuilt [provenance index](https://andrewxuturtle.github.io/mcpaudit/trust/) grouping advisory-flagged packages by outcome — registry-removed, already remediated, or still shipping the flagged version. *(★ 0 · updated 2026-08-02)*
  - **Related:** [aguara](https://github.com/garagon/aguara) · [mcp-armor](https://github.com/aira-security/mcp-armor) · [Ramparts](https://github.com/highflame-ai/ramparts)
