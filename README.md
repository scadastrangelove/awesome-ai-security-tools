...
- **[gate.cat](https://github.com/BGMLAI/gate.cat)** 🟢 — Deterministic fail-closed deny-list that vetoes an AI coding agent's irreversible shell commands (rm -rf, disk wipe, prod-DB drop, secret exfiltration, runaway fork loops) before they execute, via a pre-execution hook. — **note:** Core is Apache-2.0 (pip install gate.cat); the hosted Cloud subscription and one-time policy packs are commercial. Enforces a fixed catastrophic-command deny-list, so it complements rather than replaces sandboxing and egress-control tools. Young project with limited independent adoption signal. *(★ 3 · updated 2026-07-27)*
  - **Related:** [agentguard](https://github.com/GoPlusSecurity/agentguard) · [Parallax](https://github.com/agent-defense/parallax)
