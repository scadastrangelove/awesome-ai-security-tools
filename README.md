...
- **[node9](https://github.com/node9-ai/node9-proxy)** 🟢 — Local pre-execution gate for AI coding agents and MCP servers that allows, holds for human approval, or blocks each tool call, with a credential jail and secret detection on by default. *(Node9)* — **note:** enforcement uses each agent's own hooks and an MCP wrapper, not an isolation boundary: agents without hook support are covered only through MCP, and a script the agent runs is judged by its command line, not its contents. The egress allowlist is opt-in. Runs locally with no account; a hosted team workspace and a paid tier are optional, and `node9 init` asks before sending a one-time install ping. *(★ 216 · updated 2026-09-25)*
  - **Related:** [Pipelock](https://github.com/luckyPipewrench/pipelock) · [agentsh](https://github.com/canyonroad/agentsh)
