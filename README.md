...
- **[ccs-lint](https://github.com/DSHCorrectover/ccs-lint-action)** 🟢⚠️ — Zero-dependency CLI and GitHub Action that scans MCP/agent JSON-RPC responses for unsigned receipt field injection and forged trust indicators (_ccsReceipt, _verified, _trusted) across 5 rules, with SARIF output. — **note:** source-available Elastic License 2.0 (not OSI-approved); the project is new with limited independent adoption. CLI also published as `npx ccs-lint` on npm. *(★ 0 · updated 2026-08-24)*
